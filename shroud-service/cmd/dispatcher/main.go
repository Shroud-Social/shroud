...
func main() {
	environment, err := config.Load[config.DispatcherEnvironment]()
	if err != nil {
		panic(err)
	}
	...
	log.Println("Shroud Dispatcher service started")

	sigchan := make(chan os.Signal, 1)
	signal.Notify(sigchan, syscall.SIGINT, syscall.SIGTERM)
	<-sigchan
	log.Println("Shroud Dispatcher service shutting down...")
	...
	log.Println("Shroud Dispatcher service shut down")
}
