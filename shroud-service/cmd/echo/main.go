...
func main() {
	environment, err := config.Load[config.EchoEnvironment]()
	if err != nil {
		panic(err)
	}
	...
	log.Println("Shroud Echo service started")

	sigchan := make(chan os.Signal, 1)
	signal.Notify(sigchan, syscall.SIGINT, syscall.SIGTERM)
	<-sigchan
	log.Println("Shroud Echo service shutting down...")
	...
	log.Println("Shroud Echo service shut down")
}
